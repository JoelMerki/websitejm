...
description: Analysis of climate change and temperature anomalies
draft: false
...
Let's run the code below to load the file:
...
Let's select the releant columns and convert the datafram from wide to long format.
...
Let's consttruct a confidence interval for the average annual
...
Using some of the summary statistics we calculated, we determined the
t-critical value and margin of error. From this we were able to find the
confidence interval for the sample mean. The 95% confidence interval for
average annual temperature change is [1.02, 1.11] using both the formula
and bootstrap simulation. It means that we can be 95% confident that the
true mean temperature delta lies in this interval.The data suggests that
from 2011 to today, the annual temperature increase is above one degree,
which may significantly affect the global environment. Contrary to what
the Donald believes, the data does not lie and global warming is indeed
real.
